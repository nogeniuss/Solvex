  ...
  try {
    ...
    const investimentos = await query(`
      SELECT 
        ...
        rentabilidade as rendimento_esperado,
        data_inicio,
        status
      FROM investimentos 
    ...
    const stats = await queryOne(`
      SELECT 
        COALESCE(SUM(valor_inicial), 0) as totalInvestido,
...
router.post('/', authenticateToken, async (req, res) => {
  try {
        ...
        tipo_investimento, rentabilidade, data_resgate, instituicao, observacoes
      } = req.body;

    // Validações
    if (!titulo || !valor_inicial || !data_inicio) {
    ...
    if (rentabilidade && (rentabilidade < 0 || rentabilidade > 100)) {
      return res.status(400).json({ error: 'Rentabilidade deve estar entre 0 e 100%' });
    }

    // Inserir investimento
    const result = await execute(`
      INSERT INTO investimentos (
        titulo, descricao, valor_inicial, data_inicio, categoria_id, user_id,
        tipo, rentabilidade, data_resgate, instituicao
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      titulo, descricao || '', valor_inicial, data_inicio,
      categoria_id || null, req.user.id, tipo_investimento || 'renda_fixa',
      rentabilidade || null, data_resgate || null,
      instituicao || ''
    ]);
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
          const {
        titulo, descricao, valor_inicial, data_inicio, categoria_id,
        tipo_investimento, rentabilidade, data_resgate, instituicao, observacoes, status
      } = req.body;
    ...
    await execute(`
      UPDATE investimentos SET 
        titulo = ?, descricao = ?, valor_inicial = ?, data_inicio = ?, categoria_id = ?,
        tipo = ?, rentabilidade = ?, data_resgate = ?, 
        instituicao = ?, status = ?
      WHERE id = ?
    `, [
      titulo, descricao || '', valor_inicial, data_inicio, categoria_id || null,
      tipo_investimento || 'renda_fixa', rentabilidade || null, data_resgate || null,
      instituicao || '', status || 'ativo', id
    ]);
